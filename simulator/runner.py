"""시뮬레이터 러너 — SQLite + 더미 데이터 + 시나리오 + 이상감지.
...
def main():
    ...
    for stmt in sql.split(";"):
        ...
        if stmt:
            conn.execute(stmt)
    conn.commit()
    logger.info("MES + detection tables created")
    ...
    async def lifespan(app: FastAPI):
        ...
        logger.info("=" * 60)
        logger.info("FAB 이상감지 시뮬레이터 running")
    ...
    app = FastAPI(title="FAB 이상감지 (Simulator)", lifespan=lifespan)
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
        allow_methods=["*"],
