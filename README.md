...
├── alert/                          # 알림 (추후)
│   ├── router.py                   # 알림 라우터
│   └── escalation.py               # 에스컬레이션
│
├── correlation/                    # 상관분석 (추후)
│   └── engine.py                   # 시간/공간/인과 분석
│
└── lifecycle/                      # 이상 생명주기 (추후)
    └── manager.py                  # 상태 전이 관리
```
...
| 기능 | 설명 | 참고 파일 |
|------|------|----------|
| **연쇄 이상감지** | 다건 이상의 시간/공간/인과 관계 분석 | `correlation/engine.py` |
| **알림** | 이상 발생 시 이메일/메신저 자동 알림 | `alert/router.py` |
| **에스컬레이션** | 미확인 이상 자동 재알림 | `alert/escalation.py` |
| **이상 생명주기** | 자동 에스컬레이션 + SLA 관리 | `lifecycle/manager.py` |
